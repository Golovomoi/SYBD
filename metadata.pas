...
type
	...
	TField_Info = class
		...
		constructor create(AOwner_Table_Name, Aname, ARu_Name: String; AField_Type: TField_Type);
	...
	public
		...
		constructor create(ATable_Name, ATable_Ru_Name, AGenerator_Name: String; AReference_Table: boolean);
...
procedure Add_My_Table(ATable_Name, ATable_Ru_Name, AGenerator_Name: String; AReference_Table: Boolean);
...
begin
	...
	My_Tables[High(My_Tables)] := TTable_Info.create(ATable_Name, ATable_Ru_Name, AGenerator_Name, AReference_Table);
end;

{ TReference_Field_Info }

constructor TReference_Field_Info.create(AOwner_Table_Name, AName, 
	ARu_Name: string; AField_Type: TField_Type; AInner_Field_Name, 
	AInner_Table_Name: String);
begin
...
begin
	result:= ' inner join ' + FInner_Table_Name + ' on ' + FOwner_Table_Name + '.' + FName + ' = '
		+ FInner_Table_Name + '.id ';
end;
...
constructor TTable_Info.create(ATable_Name, ATable_Ru_Name, 
	AGenerator_Name: String; AReference_Table: boolean);
var
	i : integer;
begin
...
procedure TTable_Info.Add_Field(AName, ARu_Name: String;
...
initialization
	...
	Add_My_Table('Professors_Disciplines', 'Преподаватель - Предмет', 'Professors_Disciplines_id', true);
	My_Tables[High(My_Tables)].Add_Field('Id', 'Ай-Ди', FTInt);
	My_Tables[High(My_Tables)].Add_Field('Professor_id', 'Преподаватель', FTStr, 'Professor_Name', 'Professors');
	My_Tables[High(My_Tables)].Add_Field('Discipline_id', 'Предмет', FTSTr, 'Discipline_Name', 'Disciplines');

	Add_My_Table('Disciplines_Groups', 'Предмет - Группа', 'Disciplines_Groups_id', true);
	My_Tables[High(My_Tables)].Add_Field('Id', 'Ай-Ди', FTInt);
	My_Tables[High(My_Tables)].Add_Field('Discipline_id', 'Предмет', FTStr, 'Discipline_Name', 'Disciplines');
	My_Tables[High(My_Tables)].Add_Field('Group_id', 'Группа', FTStr, 'Group_Name', 'Groups');

	Add_My_Table('Schedule_Items', 'Расписание', 'Schedule_Items_Id', true);
	My_Tables[High(My_Tables)].Add_Field('Id', 'Ай-Ди', FTInt);
	My_Tables[High(My_Tables)].Add_Field('Subject_id', 'Предмет', FTStr, 'Discipline_Name', 'Disciplines');
	My_Tables[High(My_Tables)].Add_Field('Professor_id', 'Преподаватель', FTStr, 'Professor_Name', 'Professors');
	My_Tables[High(My_Tables)].Add_Field('Day_id', 'День Недели', FTDays, 'Day_Name', 'Days');
