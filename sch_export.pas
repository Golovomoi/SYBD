...
procedure SaveExcel (FileName : string; AGrid : TMy_Grid; HorArr, VertArr : TTitle_Array;
  ACheckGroup : TCheckGroup  );

implementation

procedure SaveExcel(FileName: string; AGrid: TMy_Grid; HorArr,
  VertArr: TTitle_Array; ACheckGroup: TCheckGroup);
 Var
  ExcelApp,
  ExcelSheet,
  ExcelCol,
...
begin
  ...
  for i:=1 to High(VertArr) do begin
    WriteLn('<tr valign = "top">');
    WriteLn(Format('<td class = "h">%s</td>', [VertArr[i].FName]));
    for j:=1 to High(HorArr) do begin
      WriteLn('<td>');
      for k:=0 to High(AGrid[i][j].FRecords) do begin
        for l:=0 to Count_Schedule_Fields - 1 do
          if (ACheckGroup.Checked[l]) and (ACheckGroup.CheckEnabled[l]) then
            WriteLn(Format('<b>%s:</b> %s<br />',[Schedule_Table.FFields[l].FRu_Name, AGrid[i][j].FRecords[k].FValues[l]]));
