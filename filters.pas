...
begin
	for i := 0 to High(FFilters_List) do
		FFilters_List[i].FFilter_Panel.Top := FFilters_List[i].FFilter_Panel.Top +
			FFilters_Scrol.tag - FFilters_Scrol.Position;
	FFilters_Scrol.Tag := FFilters_Scrol.Position;
end;


procedure TFilter_Shell.Start_Filters_scrol(Sender : TObject; var DragObject : TDragObject);
begin
		...
		begin
            ...
            begin
                ...
                result+= ' ' + FFilters_List[i].Get_Field_Name
							+ ' ' + FFilters_List[i].Get_Param + ' :' + param +
							' ' + last_case;
			end;
		end;
	end;
	if Result = ' where ' then exit('');
...
begin
	...
	begin
		Parent := FFilter_Panel;
		left := FDelete_Button.Left - FFilter_Panel.Width div 3 - 10;
		Top := 12;
...
begin
	if (FTable_Info.FFields[Fselect_Field.ItemIndex].FField_Type = FTStr) 
        or (FTable_Info.FFields[Fselect_Field.ItemIndex].FField_Type = FTDays) then
		result := 'like'
	else
