...
type
	...
	TSchedule_Edit_form = class(TForm)
		...
		procedure Schedule_GridDrawCell(Sender: TObject; aCol, aRow: Integer; aRect: TRect; 
			aState: TGridDrawState);
		procedure FormShow(Sender: TObject);
		procedure Button_ShowClick(Sender: TObject);
		procedure Schedule_GridMouseMove(Sender: TObject; Shift: TShiftState; X, 
			Y: Integer);
...
begin
	...
	begin
		CheckGroup1.Checked[i]:= not ((i = vertical_fields.Items.IndexOf(vertical_fields.Caption)) or
			(i = Horisontal_fields.Items.IndexOf(Horisontal_fields.Caption)));
		CheckGroup1.CheckEnabled[i]:= not ((i = vertical_fields.Items.IndexOf(vertical_fields.Caption)) or
			(i = Horisontal_fields.Items.IndexOf(Horisontal_fields.Caption)));
	end; 
...
begin
	...
	begin
		...
		begin
			...
			begin
				if
				    (FieldByName(Schedule_Table.FFields[V_ind].FName).AsInteger = FVertical_Fields[i].FId) and
					(FieldByName(Schedule_Table.FFields[H_ind].FName).AsInteger = FHorisontal_Fields[j].FId)
				then begin
...
begin
	...
	begin
		...
		close;
		SQL.Text:= ' select * from ' + Schedule_Table.FFields[AField_Index].Get_Source_Table
			+ '  order by id ';
		open;
		First;
		while not EOF do
...
begin
	Edit_card:= TCardEditForm.create(Schedule_Table, @Show_schedule, inttostr((Sender as TSpeedButton).Tag));
	Edit_card.Show;
end; 
...
begin
	if SaveDialog.Execute then
		case SaveDialog.FilterIndex of
			1:SaveHTML(SaveDialog.FileName, FGrid_Cells_Values, FHorisontal_Fields, FVertical_Fields, CheckGroup1 );
			2:SaveExcel(SaveDialog.FileName, FGrid_Cells_Values, FHorisontal_Fields, FVertical_Fields, CheckGroup1 );
		end;
...
begin
	...
	if (Length(FGrid_Cells_Values[Pcol][Prow].FRecords)>0) and 
		((M_Y-Cur_rect.top) div (20* Selected_fields_count) < (Length(FGrid_Cells_Values[Pcol][Prow].FRecords))) then
	begin
		change_Button.tag:=strtoint(FGrid_Cells_Values[Pcol][Prow].FRecords[((M_Y-Cur_rect.top)
			div (20* Selected_fields_count))].FValues[0]);
		Delete_Button.Tag:= strtoint(FGrid_Cells_Values[Pcol][Prow].FRecords[((M_Y-Cur_rect.top)
			div (20* Selected_fields_count))].FValues[0]); ;
		Change_Button.top:=Cur_rect.Top + (((M_Y-Cur_rect.top) div (20* Selected_fields_count))+1)*(20* Selected_fields_count) -20;
		Delete_Button.top:=Cur_rect.Top + (((M_Y-Cur_rect.top) div (20* Selected_fields_count))+1)*(20* Selected_fields_count) -40;
		Add_Buuton.top:=Cur_rect.Top + (((M_Y-Cur_rect.top) div (20* Selected_fields_count))+1)*(20* Selected_fields_count) -60;
	end;

end; 

procedure TSchedule_Edit_form.Schedule_GridDrawCell(Sender: TObject; aCol, 
	...
	begin
		...
		if length(FGrid_Cells_Values[aRow][aCol].FRecords) >0 then
			...
			begin
				...
				Schedule_Grid.Canvas.line(aRect.Left,aRect.Top + rec_count+19, aRect.Left + 300, aRect.Top + rec_count+19);
			end;
...
begin
	...
	if length(FGrid_Cells_Values[size_col][size_row].FRecords)>0 then
		Schedule_Grid.RowHeights[size_row]:=length(FGrid_Cells_Values[size_col][size_row].FRecords)
			*20*Length(Schedule_Table.FFields);

end;

procedure TSchedule_Edit_form.FormClose(Sender: TObject; 
...
procedure TSchedule_Edit_form.CheckGroup1ItemClick(Sender: TObject; 
	Index: integer);
begin
